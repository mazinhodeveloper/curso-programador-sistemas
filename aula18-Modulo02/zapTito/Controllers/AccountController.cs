...
namespace zapTito.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        /*** MÉTODO DE UPDATE DA SENHA DO USUÁRIO ***/ 
        [HttpPut("change-password")] 
        public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordRequestModelUsuario model)
        {
            try 
            {
                // 1. Busca o usuário para validar a senha antiga 
                var user = await _repositoryUsuario.GetUsuarioByTelefoneAsync(model.numeroTelefone);
                if (user == null) return Unauthorized(new { erro = true, message = "Telefone não encontrado!" }); 
                
                // 2. Valida a senha Atual (Mesma lógica do login) // Chave mestre 
                string ApiKey = "MangaRosa_Com_Leite_Mata_Todos_kkk@2026"; // Texto mestre
                string PassCombined = ComputerSha256Hash(model.senha) + ApiKey + ComputerSha256Hash(model.numeroTelefone); 

                if (!BCrypt.Net.BCrypt.Verify(PassCombined, user.senha)) return Unauthorized(new { erro = true, message = "Senha atual incorreta!" }); 

                // 3. Criando a criptografia da senha nova 
                string newPassCrip = ComputerSha256Hash(model.novaSenha) + ApiKey + ComputerSha256Hash(model.numeroTelefone); 

                string novaSenhaBCript = BCrypt.Net.BCrypt.HashPassword(newPassCrip); 

                // 4. Salva no banco 
                bool sucesso = await _repositoryUsuario.UpdateSenhaAsync(model.numeroTelefone, novaSenhaBCript); 

                if (sucesso) 
                    return Ok(new { erro = false, message = "Senha alterada com sucesso!" }); 
                
                return BadRequest(new { error = true, message = "Não foi possível atualizar a senha!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { erro = true, message = "ERRO ao trocar a senha", detalhe = ex.Message });
            }
        }
