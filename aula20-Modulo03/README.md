> Aula 20 / Módulo 03    
> Docente: Nerildo Viana da Silva       
> Data: 30/03/2026      

# Programador de Sistemas  
Período: 02/02/2026 a 27/04/2026    
Horário: Segunda, Terça, Quarta, Quinta e Sexta - 13:30 às 17:30    
https://www.sp.senac.br/senac-lapa-tito/cursos-livres/curso-de-programador-de-sistemas    
   
## Modulo II  
UC2    
   
### zapTito      
1. Criar seu novo projeto          
2. API Web do ASP.NET Core     
   - Estrutura     
     .NET 8.0 (Suporte de Longo Prazo)    
   - Tipo de autenticação     
     Nenhum     
     - Configurar para HTTPS    
     - Habilitar o suporte s OpenAPI     
     - Usar controles      
   
#### Recomandações  
- https://learn.microsoft.com/pt-br/aspnet/core/web-api/?view=aspnetcore-8.0    
- https://learn.microsoft.com/pt-br/aspnet/core/tutorials/first-web-api?view=aspnetcore-10.0&tabs=visual-studio-code     
- https://learn.microsoft.com/pt-br/aspnet/core/tutorials/web-api-help-pages-using-swagger?view=aspnetcore-8.0     
- https://learn.microsoft.com/en-us/aspnet/core/tutorials/first-web-api?view=aspnetcore-10.0&tabs=visual-studio     
      